...
import {pct,dateLabel,download} from './ui';
import type {Season,Config} from './types';
import {teamPath} from '@/lib/routes';
import {gamePathFrom,rememberGameOrigin,type GameReturn} from '@/lib/game-navigation';
import type {LiveFeed} from '@/lib/live-feed';
...
export function Forecast({data,teamId,config,returnTo,liveFeed}:{data:Season;teamId:string;config:Config;returnTo:GameReturn;liveFeed?:LiveFeed|null}){
 ...
 const schedule=liveFeed?.season===data.season?liveFeed.games:fixtures.games;
 const rows=schedule.filter(g=>!g.completed&&(g.home===teamId||g.away===teamId)).map(g=>{
   ...
   const scheduledState=(t:typeof team)=>{const prior=schedule.filter(f=>f.date<g.date&&(f.completed||f.status==='STATUS_SCHEDULED')&&(f.home===t!.id||f.away===t!.id)).map(f=>f.date).sort().at(-1);return {...t!.modelState!,lastDate:prior??t!.modelState!.lastDate}};
   ...
   return {...g,opponent:home?g.awayName:g.homeName,opponentTeam:opponent,site:g.neutral?'Neutral':home?'Home':'Away',probability:probability??null,baseline:v1?.probability??null,pick:probability==null?'Not modeled':probability>=.5?'Projected W':'Projected L',reason:!g.fbs?'Opponent outside FBS model':g.status!=='STATUS_SCHEDULED'?g.status.replace('STATUS_',''):'Outside current data coverage'};
 });
 ...
 return <section className="surface forecast-panel"><div className="panel-title"><h2>Upcoming games</h2><span className="muted">Saturday model · {data.model?.version}</span></div>
 ...
 <Table><TableHeader><TableRow>{['Date','Opponent','Site','Projected result','Win probability'].map(s=><TableHead key={s}>{s}</TableHead>)}</TableRow></TableHeader><TableBody>{rows.map(g=>{const href=gamePathFrom({id:g.id,season:data.season,awayName:g.awayName,homeName:g.homeName,neutral:g.neutral},returnTo),open=()=>{rememberGameOrigin(href,returnTo);router.push(href)};return <TableRow key={g.id} className="clickable-game-row" tabIndex={0} role="link" onClick={open} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();open()}}}><TableCell>{dateLabel(g.date)}</TableCell><TableCell>{g.opponentTeam?<Link href={teamPath(g.opponentTeam)} onClick={e=>e.stopPropagation()}><strong>{g.opponent}</strong></Link>:<strong>{g.opponent}</strong>}</TableCell><TableCell>{g.site}</TableCell><TableCell>{g.probability===null?<span className="muted">{g.reason}</span>:<span className={g.probability>=.5?'result-win':'result-loss'}>{g.pick}</span>}</TableCell><TableCell>{g.probability===null?'—':<div className="forecast-chance"><strong>{pct(g.probability)}</strong><span><i style={{width:`${g.probability*100}%`}}/></span>{g.probability>.4&&g.probability<.6&&<small>Close call</small>}</div>}</TableCell></TableRow>})}</TableBody></Table>
 ...
 <details className="season-distribution"><summary>Explore the remaining-win distribution</summary><p className="subcopy">Exact probability calculation across {modeled.length} modeled games, assuming independent results and the fixed strengths above. Non-modeled games are excluded.</p><div className="distribution-bars">{distribution.map((p,k)=><div key={k}><span>{k} wins</span><div><i style={{width:`${p/Math.max(...distribution)*100}%`}}/></div><strong>{pct(p)}</strong></div>)}</div><p className="fine-print">This is a scenario calculation, not a calibrated season forecast: shared injuries, future rating changes and correlations can shift the distribution.</p></details><div className="panel-title"><p className="fine-print">ESPN schedule status refreshed {dateLabel(liveFeed?.updatedAt??fixtures.fetchedAt)} · Dates and opponents may change.</p><Button variant="outline" onClick={()=>download(`${team.abbr}-remaining-schedule.csv`,rows)}>Export forecasts</Button></div>
 </section>;
}
