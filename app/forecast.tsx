...
export function Forecast({data,teamId,config}:{data:Season;teamId:string;config:Config}){
 ...
 if(error)return <section className="surface"><h2>The road ahead</h2><p>Schedule data could not load. Refresh to try again.</p></section>;
 if(!fixtures)return <section className="surface" role="status">Loading the remaining schedule…</section>;
 if(data.season!==fixtures.season)return <section className="surface"><h2>The road ahead</h2><p>Remaining-game forecasts are available for {fixtures.season}. Historical seasons retain their frozen pregame prediction ledger below.</p></section>;
 ...
 return <section className="surface forecast-panel"><div className="panel-title"><div><h2>The road ahead</h2><p className="subcopy">If these teams played with their current strength, who would have the edge?</p></div><span className="muted">Saturday model · {data.model?.version}</span></div>
 <div className="forecast-summary"><div><strong>{expected.toFixed(1)}</strong><span>Expected remaining wins</span><small>Across {modeled.length} modeled games</small></div><div><strong>{picks}–{modeled.length-picks}</strong><span>Most likely pick per game</span><small>Not a season-record probability</small></div><div><strong>{rows.length}</strong><span>Published games remaining</span><small>{rows.length-modeled.length} outside forecast coverage</small></div></div>
