...
export function Forecast({data,teamId,config}:{data:Season;teamId:string;config:Config}){
 ...
 <Table><TableHeader><TableRow>{['Date','Opponent','Site','Projected result','Win probability'].map(s=><TableHead key={s}>{s}</TableHead>)}</TableRow></TableHeader><TableBody>{rows.map(g=><TableRow key={g.id}><TableCell><Link href={gamePath({id:g.id,season:data.season,awayName:g.awayName,homeName:g.homeName,neutral:g.neutral})}>{dateLabel(g.date)}{!g.timeValid&&<small className="muted">Kickoff TBD</small>}</Link></TableCell><TableCell>{g.opponentTeam?<Link href={teamPath(g.opponentTeam)}><strong>{g.opponent}</strong></Link>:<strong>{g.opponent}</strong>}</TableCell><TableCell>{g.site}</TableCell><TableCell>{g.probability===null?<span className="muted">{g.reason}</span>:<Link href={gamePath({id:g.id,season:data.season,awayName:g.awayName,homeName:g.homeName,neutral:g.neutral})} className={g.probability>=.5?'result-win':'result-loss'}>{g.pick}</Link>}</TableCell><TableCell>{g.probability===null?'—':<div className="forecast-chance"><strong>{pct(g.probability)}</strong><span><i style={{width:`${g.probability*100}%`}}/></span>{g.probability>.4&&g.probability<.6&&<small>Close call</small>}</div>}</TableCell></TableRow>)}</TableBody></Table>
