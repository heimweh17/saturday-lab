...
import {gameBySlug,gameParams,recentMeetings} from '@/lib/server-catalog';
...
export default async function GamePage({params}:{params:Promise<{season:string;game:string}>}){const p=await params,game=gameBySlug(Number(p.season),p.game);if(!game)notFound();return <GamePageClient game={game} meetings={recentMeetings(game)}/>}
