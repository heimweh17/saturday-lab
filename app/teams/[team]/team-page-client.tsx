...
export function TeamPageClient({teamId,initialSeason}:{teamId:string;initialSeason:number}){
  ...
  return <main><div className="context-toolbar"><div className="context-pickers"><Picker label="Season" value={season} onChange={setSeason} options={(manifest?.seasons??[2026]).slice().reverse().map(y=>({value:String(y),label:String(y)}))}/>{currentData&&<Picker label="Snapshot" value={week} onChange={setWeek} options={currentData.weeks.map(w=>({value:String(w),label:weekName(w)}))}/>}</div><div className="snapshot-note"><span className="small-rule"/>Results through <strong>{dateLabel(snapshot?.through??null)}</strong><span>Saved snapshot</span></div></div>{error?<div className="notice">{error}</div>:!currentData||!report||!snapshot?<div className="loading">Loading team report…</div>:!team?<section className="surface empty-state"><h1>Not in the {season} FBS field</h1><p>This team does not appear in the selected season. Choose another season above.</p></section>:<Scouting key={`${season}-${week}-${teamId}`} config={report.config} data={currentData} snapshot={snapshot} week={week} selected={teamId} onSelect={id=>{const next=snapshot.teams.find(t=>t.id===id);if(next)router.push(teamPath(next))}} onCompare={id=>router.push(`/matchup/?a=${id}`)}/>}</main>;
}
