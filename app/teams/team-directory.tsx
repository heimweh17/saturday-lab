...
export function TeamDirectory(){
  ...
  return <main><header className="route-intro"><span className="scoreline">2026 FBS team directory</span><h1>Find your team</h1><p>Search by school, nickname, abbreviation or conference. Every result opens the team’s rankings, schedule, results and statistics.</p></header><section className="team-search-hero"><Search/><input autoFocus aria-label="Search teams" placeholder="Try Florida, Buckeyes, SEC…" value={query} onChange={e=>setQuery(e.target.value)}/><span>{query?`${results.length} matches`:`${teams.length||138} FBS teams`}</span></section>{error?<div className="notice">Team directory data could not load. Refresh the page to try again.</div>:!data?<div className="loading">Loading the team directory…</div>:query?<section><div className="directory-heading"><h2>Search results</h2></div><div className="team-directory-grid">{results.map(team=><TeamCard key={team.id} team={team}/>)}</div>{!results.length&&<div className="surface empty-state"><h2>No team found</h2><p>Try a school name, nickname, abbreviation or conference.</p></div>}</section>:<><section><div className="directory-heading"><h2>Featured programs</h2></div><div className="team-directory-grid featured">{popular.map(team=><TeamCard key={team.id} team={team}/>)}</div></section><section><div className="directory-heading"><h2>All FBS teams</h2></div><div className="team-directory-grid compact">{teams.map(team=><TeamCard key={team.id} team={team}/>)}</div></section></>}</main>;
}
