...
export function SearchPicker({label,value,onChange,options,placeholder='Search teams…'}:{label:string;value:string;onChange:(v:string)=>void;options:{value:string;label:string}[];placeholder?:string}){
 ...
 return <SearchPickerInput key={value} label={label} selected={selected} onChange={onChange} options={options} placeholder={placeholder}/>;
}
function SearchPickerInput({label,selected,onChange,options,placeholder}:{label:string;selected:string;onChange:(v:string)=>void;options:{value:string;label:string}[];placeholder:string}){
 const [query,setQuery]=useState(selected),listId=useId();
 const commit=(next:string)=>{setQuery(next);const match=options.find(option=>option.label.toLocaleLowerCase()===next.trim().toLocaleLowerCase());if(match)onChange(match.value)};
 return <label className="picker search-picker"><span>{label}</span><input type="search" list={listId} value={query} onChange={event=>commit(event.target.value)} onBlur={()=>{if(!options.some(option=>option.label.toLocaleLowerCase()===query.trim().toLocaleLowerCase()))setQuery(selected)}} placeholder={placeholder} aria-label={`${label}: search teams`} autoComplete="off"/><datalist id={listId}>{options.map(option=><option key={option.value} value={option.label}/>)}</datalist></label>
}
